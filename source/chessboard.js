function ChessBoard() {
  ...
  this.isValidSquare = (square) => {
    if (possibleMoves) {
      return possibleMoves.some((cell) => {
        return (
          cell.x == square.getPosition().x && cell.y == square.getPosition().y
        );
      });
    }
    return false;
  ...
  this.moveChess = (source, destination) => {
    ...
    if (
      destination.havingChessman() &&
      destination.getChessman().type == ChessmanType.KING
    ) {
      this.state = GAME_STATE.GAMEOVER;
      this.turn =
        this.turn === ColorType.TEAM.WHITE
          ? ColorType.TEAM.BLACK
          : ColorType.TEAM.WHITE;
      this.boardHTML.style.display = "none";
      gameOver(chessman.getColor());
    }
    // promote
    if (
      chessman.type === ChessmanType.PAWN &&
      (destination.position_Y === 0 || destination.position_Y === 7)
    ) {
    ...
    if (
      ...
      chessman.type === ChessmanType.ROOK
    ) {
      chessman.moved();
    }
  ...
  this.isAbleToCastling = (source, destination) => {
    if (
      source.getChessman().getColor() !== destination.getChessman().getColor()
    ) {
      return false;
    }
    ...
    if (
      (sourceType === ChessmanType.KING && desType === ChessmanType.ROOK) ||
      (desType === ChessmanType.KING && sourceType === ChessmanType.ROOK)
    ) {
      if (
        source.getChessman().hasMoved() ||
        destination.getChessman().hasMoved()
      ) {
        return false;
      }
      let direct =
        source.getPosition().x > destination.getPosition().x ? -1 : 1;

      for (
        let x = source.getPosition().x + direct;
  ...
  this.castle = (kingSquare, rookSquare) => {
    ...
    let direct =
      kingSquare.getPosition().x > rookSquare.getPosition().x ? -1 : 1;
  ...
  this.handleClickChessman = (square) => {
    if (!secondClick) {
      ...
      if (
        square.havingChessman() &&
        square.getChessman().getColor() === this.turn
      ) {
        secondClick = true;
        square.select(true);
      ...
      if (
        square.havingChessman() &&
        this.isAbleToCastling(this.selectedSquare, square)
      ) {
  ...
  this.checkCheckmated = (chessmap) => {
    chessmap.forEach((row) => {
      row.forEach((square) => {
        if (
          square.havingChessman() &&
          square.getChessman().type == ChessmanType.KING
        ) {
